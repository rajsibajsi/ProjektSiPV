    ...
    <div class="row">
        <div class="col-md-10 col-md-offset-1">
            <div class="panel panel-default">
                <div class="panel-heading"><div class="row"><div class="col-sm-10">Users</div><div style="text-align:left; cursor: pointer;" class="col-sm-2"><span style="padding-top: 3px;" class="glyphicon glyphicon-map-marker"></span><div style="padding-right: 6px; padding-top: 3px;" id="nearMeButton">Near me</div><input style="float: right; max-width: 50px;" value="50" type="text" id="distanceValue"></div></div></div>

                <div class="panel-body">
                    @if( ! empty($users) )
                        @foreach ($users as $user)
                            <a style="text-decoration: none;" href="{{ url('profile/' . $user->id) }}"><h4 style="color: black; padding-bottom: 20px; padding-top: 10px; border-bottom: 1px solid #f5f8fa;">{{ $user->name }}</h4><span style="display: hidden;" class="distance">{{ $user->distanceToMe }}</span></a>
                        @endforeach
                    @endif
                </div>
            </div>
